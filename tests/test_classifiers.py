...
class ClassifiersTest(unittest.TestCase):
    ...
    def test_cld2_predict(self):
        runner = CliRunner()
        result = runner.invoke(lidtk.classifiers.cld2_mod.predict_cli,
                               ['--text', 'I don\'t go to school.'])
        self.assertEqual(result.output, 'eng\n')

    def test_cld2_get_languages(self):
        runner = CliRunner()
        runner.invoke(lidtk.classifiers.cld2_mod.get_languages, [])

    def test_cld2_eval_willi(self):
        runner = CliRunner()
        runner.invoke(lidtk.classifiers.cld2_mod.eval_wili, [])
